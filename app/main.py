...
NODE_AI_SINGLE_SERVICE = os.getenv("NODE_AI_SINGLE_SERVICE", "true").strip().lower() in {"1", "true", "yes", "on"}
NODE_AI_URL = os.getenv("NODE_AI_URL", "http://127.0.0.1:3100").strip().rstrip("/")
if NODE_AI_SINGLE_SERVICE:
    NODE_AI_URL = "http://127.0.0.1:3100"
NODE_AI_TIMEOUT_SECONDS = float(os.getenv("NODE_AI_TIMEOUT_SECONDS", "55"))
...
def dashboard_timeseries(db: Session = Depends(get_db), user: User = Depends(current_user)):
    """Realtime dashboard series: hourly traffic for today and daily revenue for 7 days."""
    now = now_vn()
    today = now.date()
    start = datetime.combine(today, datetime.min.time())
    end = start + timedelta(days=1)
    hours = list(range(24))
    ins = {h: 0 for h in hours}
    outs = {h: 0 for h in hours}
    for (dt,) in db.query(ParkingRecord.time_in).filter(ParkingRecord.time_in >= start, ParkingRecord.time_in < end).all():
        ins[dt.hour] += 1
    for (dt,) in db.query(ParkingRecord.time_out).filter(ParkingRecord.time_out >= start, ParkingRecord.time_out < end).all():
        outs[dt.hour] += 1
    daily = {}
    since = today - timedelta(days=6)
    rows = db.query(ParkingRecord.time_out, ParkingRecord.fee).filter(ParkingRecord.time_out.is_not(None), ParkingRecord.time_out >= datetime.combine(since, datetime.min.time()), ParkingRecord.time_out < end).all()
    for dt, fee in rows:
        key = dt.strftime("%d/%m")
        daily[key] = daily.get(key, 0.0) + float(fee or 0)
    for dt, fee in db.query(MonthlyPass.started_at, MonthlyPass.price).filter(MonthlyPass.started_at >= datetime.combine(since, datetime.min.time()), MonthlyPass.started_at < end).all():
        key = dt.strftime("%d/%m")
        daily[key] = daily.get(key, 0.0) + float(fee or 0)
    days = [(since + timedelta(days=i)).strftime("%d/%m") for i in range(7)]
    return {"generated_at": now.isoformat(), "hours": hours, "checkins": [ins[h] for h in hours], "checkouts": [outs[h] for h in hours], "days": days, "revenue": [daily.get(k, 0.0) for k in days]}
...
def delete_area(area_id: int, db: Session = Depends(get_db), user: User = Depends(manager_only)):
    ...
    if slot_ids:
        active_records = db.query(ParkingRecord).filter(
            ...
            ParkingRecord.time_out.is_(None)
        ).all()
    if active_records:
        raise HTTPException(
            409,
            f"Không thể xóa {area.name} vì đang có {len(active_records)} xe trong khu. Hãy cho xe ra trước."
        )
    ...
    try:
        ...
        if slot_ids:
            ...
            if record_ids:
                db.query(Payment).filter(
                    Payment.record_id.in_(record_ids)
                ).delete(synchronize_session=False)

                db.query(ParkingRecord).filter(
                    ParkingRecord.id.in_(record_ids)
                ).delete(synchronize_session=False)

            db.query(ParkingSlot).filter(
                ParkingSlot.id.in_(slot_ids)
            ).delete(synchronize_session=False)
        ...
        audit(
            db, user, "DELETE_AREA",
            f"Xóa {old_name} và {len(slot_ids)} vị trí cùng lịch sử liên quan"
        )
        db.commit()

        return {"message": f"Đã xóa {old_name}", "deleted_area_id": area_id}

    except Exception as exc:
        db.rollback()
        raise HTTPException(500, f"Không thể xóa khu vực: {str(exc)}")
