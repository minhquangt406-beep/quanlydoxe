...
function destroyDashCharts(){Object.values(window.__dashCharts||{}).forEach(c=>{try{c.destroy()}catch(_){}});window.__dashCharts={};}
function initDashCharts(ch){
  if(typeof Chart==='undefined') return;
  destroyDashCharts();
  const base={responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{usePointStyle:true,boxWidth:8,font:{size:10}}},tooltip:{padding:10,cornerRadius:10,displayColors:true}},scales:{x:{grid:{display:false},ticks:{font:{size:9},maxRotation:0}},y:{grid:{color:'#edf0f5'},ticks:{font:{size:9}}}}};
  const c1=document.getElementById('liveFlowChart');
  if(c1) window.__dashCharts.flow=new Chart(c1,{type:'line',data:{labels:ch.hourly.labels,datasets:[{label:'Xe vào',data:ch.hourly.checkins,borderWidth:3,tension:.42,pointRadius:2,fill:true},{label:'Xe ra',data:ch.hourly.checkouts,borderWidth:2,tension:.42,pointRadius:2}]},options:{...base,plugins:{...base.plugins,title:{display:false}},scales:{...base.scales,y:{...base.scales.y,beginAtZero:true,ticks:{...base.scales.y.ticks,stepSize:1}}}}});
  const c2=document.getElementById('dailyRevenueChart');
  if(c2) window.__dashCharts.revenue=new Chart(c2,{type:'bar',data:{labels:ch.days.map(x=>x.date),datasets:[{label:'Doanh thu',data:ch.days.map(x=>x.revenue),borderRadius:8,maxBarThickness:28}]},options:{...base,plugins:{...base.plugins,tooltip:{...base.plugins.tooltip,callbacks:{label:ctx=>money(ctx.raw)}}},scales:{...base.scales,y:{...base.scales.y,beginAtZero:true,ticks:{...base.scales.y.ticks,callback:v=>money(v).replace(' ₫','')}}}}});
  const c3=document.getElementById('vehicleMixChart');
  if(c3) window.__dashCharts.mix=new Chart(c3,{type:'doughnut',data:{labels:Object.keys(ch.vehicle_types),datasets:[{data:Object.values(ch.vehicle_types),borderWidth:0,hoverOffset:8}]},options:{responsive:true,maintainAspectRatio:false,cutout:'70%',plugins:{legend:{position:'bottom',labels:{usePointStyle:true,boxWidth:8,font:{size:9}}}}}});
}
async function refreshDashboardLive(){
  if(!me || !document.querySelector('[data-page="dashboard"].active') || !document.getElementById('liveFlowChart')) return;
  try{
    const [d,ch]=await Promise.all([api('/api/dashboard'),api('/api/dashboard/chart-data')]);
    const cards=document.querySelectorAll('#content .cards .card .metric');
    if(cards[0])cards[0].textContent=d.active_vehicles;
    if(cards[1])cards[1].textContent=d.empty;
    if(cards[2])cards[2].textContent=d.today_checkins;
    if(cards[3])cards[3].textContent=d.today_checkouts;
    if(cards[4])cards[4].textContent=money(d.revenue||d.today_revenue||0);
    if(cards[5])cards[5].textContent=d.occupancy_rate+'%';
    initDashCharts(ch);
    const stamp=document.getElementById('dashboardLiveStamp'); if(stamp) stamp.textContent='● LIVE · cập nhật '+new Date().toLocaleTimeString('vi-VN');
  }catch(e){console.warn('Live dashboard refresh:',e)}
}
clearInterval(window.__dashLiveTimer);window.__dashLiveTimer=setInterval(refreshDashboardLive,10000);
