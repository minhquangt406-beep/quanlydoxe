...
/* V29 start-login fallback: delegated listener guarantees the button works. */
(function(){
  const start=document.getElementById('v21Start');
  const root=document.getElementById('loginView');
  const auth=document.getElementById('v21Auth');
  if(!start || !root || !auth || start.dataset.v29StartFallback) return;
  start.dataset.v29StartFallback='1';
  start.addEventListener('click', function(){
    root.classList.add('v28-auth-open');
    auth.setAttribute('aria-hidden','false');
