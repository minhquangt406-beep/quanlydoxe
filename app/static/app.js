...
window.pages.activity=async()=>{const log=await api('/api/activity?limit=100');const scope=me?.role==='manager'||me?.role==='admin'?'toàn hệ thống':'của tài khoản này';$('#content').innerHTML=`<div class="panel"><div class="panel-head"><div><h3>Nhật ký hoạt động</h3><span class="muted">100 thao tác gần nhất · ${scope} · dữ liệu lưu trong PostgreSQL</span></div><span class="pill green">AUDIT LOG</span></div><div class="activity-list">${log.length?log.map(activityItem).join(''):'<div class="empty-state">Chưa có dữ liệu</div>'}</div></div>`}
