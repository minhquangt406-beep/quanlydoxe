...
function maybeReturnToAdmin(){const backup=sessionStorage.getItem("parking_admin_return_token");if(!backup||!me||me.role==="manager"||me.role==="admin")return;const nav=document.querySelector("#mobileBottomNav");if(document.querySelector("#returnAdminBtn"))return;const btn=document.createElement("button");btn.id="returnAdminBtn";btn.className="return-admin-btn";btn.textContent="↩ Về tài khoản quản lý";btn.onclick=()=>{localStorage.setItem("parking_token",backup);sessionStorage.removeItem("parking_admin_return_token");sessionStorage.removeItem("parking_admin_return_user");location.reload()};document.body.appendChild(btn);}
...
function openSlotModal(slot){
    ...
    <div class="slot-modal-shell">
      ...
      ${occupied ? `
        <div class="slot-status occupied">
          ...
          <div><b>VỊ TRÍ ĐANG SỬ DỤNG</b><small>${displayPlate(slot.license_plate, slot.vehicle_type)}</small></div>
          <span class="status-dot"></span>
        </div>
        <div class="slot-info-grid">
          <div><span>BIỂN SỐ</span><b>${displayPlate(slot.license_plate, slot.vehicle_type)}</b></div>
  ...
  if(occupied){
    ...
    const update=()=>{
      ...
      plateInput.value=isBike ? "" : plateInput.value;
      const field=$("#quickPlateField");
      if(field) field.classList.toggle("is-bike", isBike);
      const hint=$("#quickPlateHint");
      if(hint) hint.textContent=isBike ? "Xe đạp không cần nhập biển số" : "Ví dụ: 30A12345 → 30A-123.45";
      const raw=plateInput.value||"";
      const clean=raw.toUpperCase().replace(/[^A-Z0-9]/g,"");
      const detected=isBike ? "Xe đạp" : (clean.length>=5 ? detectVehicleType(clean) : "");
      if(detected){
        typeSelect.value=detected;
        typeSelect.dataset.autoDetected="1";
...
async function openPaymentModal(recordId){
  ...
  try{
    ...
    const plate=displayPlate(d.license_plate, d.vehicle_type);
...
function vehicleIcon(type){
  ...
  if(t.includes('đạp')) return '<svg class="vehicle-svg bicycle-svg" viewBox="0 0 64 64" aria-label="Xe đạp"><circle cx="17" cy="47" r="10" fill="none" stroke="currentColor" stroke-width="3.5"/><circle cx="47" cy="47" r="10" fill="none" stroke="currentColor" stroke-width="3.5"/><path d="M17 47l12-21h10l8 21M29 26l8 21M29 26h-7M37 47h10M39 26h8l4 6M29 26l-4-7h8" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
$("#content").innerHTML=`<div class="dash-head"><div><div class="eyebrow">BẢNG ĐIỀU KHIỂN THỜI GIAN THỰC</div><h1>Tổng quan vận hành</h1><span class="muted">Theo dõi bãi xe, lưu lượng và doanh thu trong một màn hình.</span></div><div class="realtime-badge"><i></i><b>LIVE</b><span id="dashUpdated">Đang đồng bộ...</span></div></div>
<div class="cards dashboard-kpis"><div class="card kpi-card"><div class="label">XE ĐANG GỬI</div><div class="metric" id="kpiActive">${d.active_vehicles}</div><div class="trend positive">● Đang vận hành</div></div><div class="card kpi-card"><div class="label">Ô TRỐNG</div><div class="metric" id="kpiEmpty">${d.empty}</div><div class="trend">${d.total_slots} tổng vị trí</div></div><div class="card kpi-card"><div class="label">XE VÀO HÔM NAY</div><div class="metric" id="kpiIn">${d.today_checkins}</div><div class="trend">Cập nhật realtime</div></div><div class="card kpi-card"><div class="label">XE RA HÔM NAY</div><div class="metric" id="kpiOut">${d.today_checkouts}</div><div class="trend">Cập nhật realtime</div></div><div class="card kpi-card revenue-kpi"><div class="label">DOANH THU HÔM NAY</div><div class="metric" id="kpiRevenue">${money(d.today_revenue)}</div><div class="trend positive">● Đang ghi nhận</div></div><div class="card kpi-card"><div class="label">TỶ LỆ LẤP ĐẦY</div><div class="metric" id="kpiOccupancy">${d.occupancy_rate}%</div><div class="mini-progress"><i style="width:${d.occupancy_rate}%"></i></div></div></div>
<div class="realtime-charts"><div class="panel realtime-chart"><div class="chart-head"><div><h3>Lưu lượng xe theo giờ</h3><span class="muted">Xe vào và xe ra trong ngày hôm nay</span></div><span class="chart-live">● LIVE</span></div><div class="line-chart" id="trafficChart"></div><div class="chart-legend"><span><i class="ci-in"></i>Xe vào</span><span><i class="ci-out"></i>Xe ra</span></div></div><div class="panel realtime-chart"><div class="chart-head"><div><h3>Doanh thu 7 ngày</h3><span class="muted">Doanh thu gửi xe + vé tháng</span></div><span class="chart-total" id="chartTotal">${money(ts.revenue.reduce((a,b)=>a+b,0))}</span></div><div class="bar-chart" id="revenueChart"></div><div class="chart-foot"><span>Hôm nay</span><b id="chartTodayRevenue">${money(ts.revenue[6]||0)}</b></div></div></div>
<div class="panel occupancy-live-panel"><div class="chart-head"><div><h3>Trạng thái bãi xe</h3><span class="muted">Phân bổ vị trí theo thời gian thực</span></div><span class="live-clock" id="liveClock">--:--:--</span></div><div class="occupancy-layout"><div class="occupancy-ring" id="occupancyRing"><div><b>${d.occupancy_rate}%</b><span>lấp đầy</span></div></div><div class="occ-stats"><div><i class="occ-dot used"></i><span>Đang sử dụng</span><b id="occUsed">${d.occupied}</b></div><div><i class="occ-dot free"></i><span>Còn trống</span><b id="occFree">${d.empty}</b></div><div><i class="occ-dot total"></i><span>Tổng vị trí</span><b>${d.total_slots}</b></div></div><div class="occupancy-bars"><div><span><b>Khu A</b><em id="areaAValue">—</em></span><div><i id="areaABar"></i></div></div><div><span><b>Khu B</b><em id="areaBValue">—</em></span><div><i id="areaBBar"></i></div></div></div></div></div>
...
renderTraffic();renderRevenue();const areaA=areas.find(x=>x.name.toLowerCase().includes('a')),areaB=areas.find(x=>x.name.toLowerCase().includes('b'));const setArea=(el,bar,ar)=>{if(!ar)return;const occ=ar.slots.filter(x=>x.status==='occupied').length,rate=ar.slots.length?occ/ar.slots.length*100:0;if(el)el.textContent=Math.round(rate)+'%';if(bar)bar.style.width=rate+'%'};setArea($('#areaAValue'),$('#areaABar'),areaA);setArea($('#areaBValue'),$('#areaBBar'),areaB);$('#openActivity')?.addEventListener('click',()=>navigate('activity'));window.clearInterval(window.__durationTimer);window.__durationTimer=setInterval(()=>$$('.duration').forEach(e=>e.textContent=duration(e.dataset.time)),30000);window.clearInterval(window.__dashboardRealtime);window.__dashboardRealtime=setInterval(refreshDashboardRealtime,10000);window.clearInterval(window.__clockTimer);window.__clockTimer=setInterval(()=>{$('#liveClock')&&($('#liveClock').textContent=new Date().toLocaleTimeString('vi-VN'))},1000);$('#liveClock').textContent=new Date().toLocaleTimeString('vi-VN');wireMapInteractions()},
...
slots:async()=>{let s=await api("/api/slots");window.__latestSlots=s; const areas=[...new Map(s.map(x=>[x.area_id,{id:x.area_id,name:x.area_name,slots:[]}])).values()]; s.forEach(x=>areas.find(ar=>ar.id===x.area_id)?.slots.push(x)); const zone=(ar)=>{const occupied=ar.slots.filter(x=>x.status==="occupied").length,empty=ar.slots.length-occupied;const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a";return `<div class="panel area-panel-large"><div class="parking-map"><div class="map-zone ${cls}" data-area-id="${ar.id}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ vị trí</h3><span class="muted">${ar.slots.length} vị trí · ${empty} trống · ${occupied} đang dùng</span></div><span class="pill ${empty?"green":"red"}">${empty?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid">${ar.slots.map(x=>{const vt=x.vehicle_type||"Ô tô";const vcls=vt.toLowerCase().includes("đạp")?"vehicle-bicycle":(vt.toLowerCase().includes("máy")?"vehicle-bike":"vehicle-car");return `<div class="real-slot ${x.status} ${x.status==="occupied"?vcls:""}" data-slot-id="${x.id}" data-area-id="${ar.id}"><div class="slot-top"><span>${x.name}</span><span>${x.status==="occupied"?vt:""}</span></div><div class="slot-icon">${x.status==="occupied"?vehicleIcon(vt):emptyIcon()}</div>${x.status==="occupied"?`<span class="plate">${displayPlate(x.license_plate, x.vehicle_type)}</span>`:`<span class="slot-status">CHỖ TRỐNG</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG SỬ DỤNG":"SẴN SÀNG"}</span></div>`}).join("")}</div></div></div></div>`}; $("#content").innerHTML=`<div class="notice">Bản đồ được chia riêng theo từng khu. <b>Biển số</b> nằm ngay trên ô đang sử dụng.</div><div class="area-list">${areas.map(zone).join("")}</div>`},
history:async()=>{let h=await api("/api/history");
const methodKey=row=>{if(!row.time_out)return "pending";const m=String(row.payment_method||"");if(m==="QR ngân hàng"||m==="Chuyển khoản")return "bank";if(m==="Tiền mặt")return "cash";if(m==="Miễn phí")return "free";return "other"};
const methodLabel=k=>k==="bank"?"Chuyển khoản & QR":k==="cash"?"Tiền mặt":k==="free"?"Miễn phí":k==="pending"?"Chưa thanh toán":"Khác";
const methodClass=k=>k==="bank"?"payment-bank":k==="cash"?"payment-cash":k==="free"?"payment-free":k==="pending"?"payment-pending":"payment-other";
const methodIcon=k=>k==="bank"?"🏦":k==="cash"?"💵":k==="free"?"🎁":k==="pending"?"⏳":"•";
const rowHtml=row=>`<tr><td>#${row.id}</td><td><b>${displayPlate(row.license_plate, row.vehicle_type)}</b></td><td>${row.vehicle_type}</td><td>${row.slot}</td><td>${dt(row.time_in)}</td><td>${dt(row.time_out)}</td><td>${row.fee!=null?money(row.fee):"—"}</td><td><span class="payment-badge ${methodClass(methodKey(row))}">${methodIcon(methodKey(row))} ${methodLabel(methodKey(row))}</span></td><td>${row.paid_at?dt(row.paid_at):"—"}</td><td><span class="pill ${row.time_out?"green":"red"}">${row.time_out?"Đã ra":"Đang gửi"}</span></td>${me?.role==="manager"?`<td><button class="btn danger delete-history" data-id="${row.id}">🗑 Xóa</button></td>`:""}</tr>`;
const renderRows=(rows, key)=>rows.filter(r=>key==="all"||methodKey(r)===key).map(rowHtml).join("");
const count=k=>h.filter(r=>k==="all"||methodKey(r)===k).length;
const total=k=>h.filter(r=>k==="all"||methodKey(r)===k).reduce((a,r)=>a+Number(r.fee||0),0);
const tab=(key,label,icon)=>`<button class="payment-history-tab ${key==="all"?"active":""}" data-pay-filter="${key}"><span>${icon}</span><b>${label}</b><em>${count(key)}</em><small>${money(total(key))}</small></button>`;
const tableHead=`<table class="table"><thead><tr><th>Mã</th><th>Biển số</th><th>Loại</th><th>Vị trí</th><th>Vào</th><th>Ra</th><th>Tiền</th><th>Phương thức</th><th>Thanh toán lúc</th><th>Trạng thái</th>${me?.role==="manager"?"<th></th>":""}</tr></thead><tbody id="hist"></tbody></table>`;
const render=(rows=h,key="all")=>{$("#hist").innerHTML=renderRows(rows,key);bindDelete();};
const bindDelete=()=>$$('.delete-history').forEach(b=>b.onclick=async()=>{if(!confirm(`Xóa lượt #${b.dataset.id}?\n\nDữ liệu lượt gửi này sẽ bị xóa khỏi lịch sử.`))return;try{const d=await api(`/api/history/${b.dataset.id}`,{method:"DELETE"});toast(d.message);await pages.history()}catch(e){toast(e.message,"error")}});
const searchRows=()=>{const q=(($(' #search')||$("#search"))?.value||"").toLowerCase().trim();return h.filter(r=>`${r.id} ${r.license_plate} ${r.vehicle_type} ${r.slot} ${r.payment_method}`.toLowerCase().includes(q));};
$("#content").innerHTML=`<div class="panel payment-history-panel"><div class="panel-head"><div><h3>Lịch sử thanh toán</h3><span class="muted">Phân loại riêng để kiểm tra doanh thu và phương thức thanh toán</span></div><input id="search" style="width:240px;margin:0" placeholder="Tìm biển số, mã lượt..."></div><div class="payment-history-tabs">${tab("all","Tất cả","📋")}${tab("bank","Chuyển khoản & QR","🏦")}${tab("cash","Tiền mặt","💵")}${tab("free","Miễn phí","🎁")}${tab("pending","Chưa thanh toán","⏳")}</div><div class="payment-history-summary"><span>Đang xem: <b id="paymentFilterLabel">Tất cả</b></span><span>Tổng tiền: <b id="paymentFilterTotal">${money(total("all"))}</b></span></div><div class="table-wrap">${tableHead}</div></div>`;
let activeFilter="all";render(h,activeFilter);
const updateView=()=>{const rows=searchRows();render(rows,activeFilter);const label=methodLabel(activeFilter);$("#paymentFilterLabel").textContent=label;$("#paymentFilterTotal").textContent=money(rows.filter(r=>activeFilter==="all"||methodKey(r)===activeFilter).reduce((a,r)=>a+Number(r.fee||0),0));$$('.payment-history-tab').forEach(b=>b.classList.toggle('active',b.dataset.payFilter===activeFilter));};
$$('.payment-history-tab').forEach(b=>b.onclick=()=>{activeFilter=b.dataset.payFilter;updateView()});
...
window.pages.reports=async()=>{const d=await api('/api/reports?days=30');const rev=await api('/api/revenue');const rh=await api('/api/revenue/history');const daily=Object.entries(d.daily).sort((a,b)=>b[0].localeCompare(a[0]));const latestDaily=daily[0]||null;const max=Math.max(1,...daily.map(x=>x[1]));const bars=daily.slice(0,14).map(([day,val])=>`<div class="report-bar-row"><span>${day.slice(5)}</span><div class="report-bar"><i style="width:${Math.max(2,val/max*100)}%"></i></div><b>${money(val)}</b></div>`).join('');const areas=Object.entries(d.by_area).sort((a,b)=>b[1]-a[1]).map(([name,val])=>`<tr><td>${name}</td><td><b>${money(val)}</b></td></tr>`).join('');$('#content').innerHTML=`<div class="cards"><div class="card"><div class="label">DOANH THU GẦN ĐÂY NHẤT</div><div class="metric">${latestDaily?money(latestDaily[1]):"0 ₫"}</div><div class="trend">${latestDaily?"Ngày "+latestDaily[0]:"Chưa có dữ liệu"}</div></div><div class="card"><div class="label">DOANH THU THÁNG NÀY</div><div class="metric">${money(rev.current_revenue)}</div><div class="trend">Tự động reset mỗi tháng · ${rev.period}</div></div><div class="card"><div class="label">DOANH THU 30 NGÀY</div><div class="metric">${money(d.total_revenue)}</div><div class="trend">Theo lượt đã hoàn tất</div></div><div class="card"><div class="label">LƯỢT ĐÃ THANH TOÁN</div><div class="metric">${d.closed_records}</div><div class="trend">Trong 30 ngày gần nhất</div></div></div><div class="panel revenue-control-panel"><div class="panel-head"><div><h3>💰 Quản lý số tiền</h3><span class="muted">Bộ đếm doanh thu hiện tại sẽ tự reset vào tháng mới</span></div><button class="btn danger" id="resetRevenue">↻ Reset số tiền</button></div><div class="revenue-current"><b>${money(rev.current_revenue)}</b><span>Đang ghi nhận · ${rev.period}</span></div><div class="panel-head" style="margin-top:12px"><div><h3>📜 Lịch sử reset doanh thu</h3><span class="muted">Lưu riêng các mốc tháng và lần reset thủ công</span></div></div><div class="table-wrap"><table class="table"><thead><tr><th>Thời gian</th><th>Số tiền đã chốt</th></tr></thead><tbody>${rh.map(x=>`<tr><td>${x.period}</td><td><b>${money(x.amount)}</b></td></tr>`).join('')||'<tr><td colspan="2">Chưa có lịch sử reset</td></tr>'}</tbody></table></div></div><div class="grid2"><div class="panel"><div class="panel-head"><div><h3>Doanh thu theo ngày</h3><span class="muted">14 ngày gần nhất có dữ liệu</span></div><span class="pill green">BUSINESS</span><button class="btn" id="exportHistory">⇩ Xuất CSV</button></div><div class="report-bars">${bars||'<div class="empty-state">Chưa có dữ liệu doanh thu</div>'}</div></div><div class="panel"><div class="panel-head"><h3>Doanh thu theo khu</h3></div><table class="table"><thead><tr><th>Khu</th><th>Doanh thu</th></tr></thead><tbody>${areas||'<tr><td colspan="2">Chưa có dữ liệu</td></tr>'}</tbody></table></div></div>`;$('#resetRevenue').onclick=async()=>{const btn=$('#resetRevenue');if(!confirm('Reset số tiền doanh thu hiện tại? Lịch sử thanh toán sẽ không bị xóa.'))return;try{btn.disabled=true;btn.textContent='↻ Đang reset...';const result=await api('/api/revenue/reset',{method:'POST'});toast(result.message||'Đã reset số tiền');if($('#resetRevenue'))$('#resetRevenue').disabled=false;await pages.reports()}catch(e){toast(e.message,'error');btn.disabled=false;btn.textContent='↻ Reset số tiền'}}}
window.pages.users=async()=>{const u=await api('/api/users');const roleLabel=x=>x.role==='manager'?'Quản lý':(x.role==='guest'?'Khách':'Nhân viên');$('#content').innerHTML=`<div class="grid2"><div class="panel"><div class="panel-head"><div><h3>Tạo tài khoản nhân sự</h3><span class="muted">Phân quyền Quản lý / Nhân viên</span></div></div><div class="form-grid"><label>Tài khoản<input id="newUser" placeholder="nhanvien01"></label><label>Mật khẩu<input id="newPass" type="password" placeholder="Tối thiểu 8 ký tự"></label><label>Họ tên<input id="newName" placeholder="Nguyễn Văn A"></label><label>Vai trò<select id="newRole"><option value="staff">Nhân viên</option><option value="manager">Quản lý</option></select></label></div><button class="primary" id="createUser">+ Tạo tài khoản</button></div><div class="panel account-admin-panel"><div class="panel-head"><div><h3>Quản lý & đăng nhập nhanh</h3><span class="muted">Quản lý có thể đổi mật khẩu hoặc đăng nhập nhanh vào bất kỳ tài khoản nào.</span></div><span class="pill green">ADMIN</span></div><div class="quick-login-grid">${u.map(x=>`<div class="quick-account"><div class="quick-account-main"><span class="quick-avatar">${x.role==='manager'?'Q':(x.role==='guest'?'K':'NV')}</span><div><b>${x.username}</b><small>${x.full_name} · ${roleLabel(x)}</small></div></div><div class="quick-account-actions"><button class="btn primary-soft login-as-user" data-id="${x.id}" ${x.id===me.id?'disabled':''}>↪ Đăng nhập</button><button class="btn password-user" data-id="${x.id}" data-username="${x.username}">🔑 Mật khẩu</button>${x.id===me.id?'<span class="muted current-account">Đang dùng</span>':''}</div></div>`).join('')}</div></div><div class="panel"><div class="panel-head"><h3>Tài khoản hệ thống</h3><span class="muted">${u.length} tài khoản</span></div><div class="table-wrap"><table class="table"><thead><tr><th>Tài khoản</th><th>Họ tên</th><th>Vai trò</th><th>Thao tác</th></tr></thead><tbody>${u.map(x=>`<tr><td><b>${x.username}</b></td><td>${x.full_name}</td><td><span class="pill ${x.role==='manager'?'green':''}">${roleLabel(x)}</span></td><td><div class="account-actions"><button class="btn primary-soft login-as-user" data-id="${x.id}" ${x.id===me.id?'disabled':''}>Đăng nhập</button><button class="btn password-user" data-id="${x.id}" data-username="${x.username}">Đổi mật khẩu</button>${x.id===me.id?'<span class="muted">Đang dùng</span>':`<button class="btn danger del-user" data-id="${x.id}">Xóa</button>`}</div></td></tr>`).join('')}</tbody></table></div></div></div><div id="userPasswordModal" class="modal hidden"><div class="modal-card"><div class="modal-head"><div><h3>Đổi mật khẩu tài khoản</h3><span id="passwordTargetLabel" class="muted"></span></div><button class="btn" id="closeUserPassword">✕</button></div><label>Mật khẩu mới<input id="adminNewUserPassword" type="password" minlength="8" placeholder="Tối thiểu 8 ký tự"></label><label>Xác nhận mật khẩu<input id="adminNewUserPassword2" type="password" minlength="8" placeholder="Nhập lại mật khẩu"></label><div class="modal-actions"><button class="btn" id="cancelUserPassword">Hủy</button><button class="primary" id="saveUserPassword">Lưu mật khẩu</button></div></div></div>`;$('#createUser').onclick=async()=>{try{await api('/api/users',{method:'POST',body:{username:$('#newUser').value,password:$('#newPass').value,full_name:$('#newName').value,role:$('#newRole').value}});toast('Đã tạo tài khoản');await pages.users()}catch(e){toast(e.message,'error')}};$$('.del-user').forEach(b=>b.onclick=async()=>{if(!confirm('Xóa tài khoản này?'))return;try{await api('/api/users/'+b.dataset.id,{method:'DELETE'});toast('Đã xóa tài khoản');await pages.users()}catch(e){toast(e.message,'error')}});let passwordUserId=null;const closePassword=()=>$('#userPasswordModal')?.classList.add('hidden');$$('.password-user').forEach(b=>b.onclick=()=>{passwordUserId=Number(b.dataset.id);$('#passwordTargetLabel').textContent='Tài khoản: '+b.dataset.username;$('#adminNewUserPassword').value='';$('#adminNewUserPassword2').value='';$('#userPasswordModal').classList.remove('hidden');setTimeout(()=>$('#adminNewUserPassword').focus(),50)});$('#closeUserPassword').onclick=closePassword;$('#cancelUserPassword').onclick=closePassword;$('#saveUserPassword').onclick=async()=>{const a=$('#adminNewUserPassword').value,b=$('#adminNewUserPassword2').value;if(a.length<8){toast('Mật khẩu mới phải có ít nhất 8 ký tự','error');return}if(a!==b){toast('Mật khẩu xác nhận không khớp','error');return}try{await api('/api/users/'+passwordUserId+'/password',{method:'POST',body:{new_password:a}});closePassword();toast('Đã đổi mật khẩu thành công')}catch(e){toast(e.message,'error')}};$$('.login-as-user').forEach(b=>b.onclick=async()=>{const id=Number(b.dataset.id);const target=u.find(x=>x.id===id);if(!target||target.id===me.id)return;if(!confirm(`Đăng nhập nhanh vào tài khoản "${target.username}" (${roleLabel(target)})?`))return;try{sessionStorage.setItem('parking_admin_return_token',token);sessionStorage.setItem('parking_admin_return_user',JSON.stringify(me));const d=await api('/api/users/'+id+'/login-as',{method:'POST'});localStorage.setItem('parking_token',d.access_token);location.reload()}catch(e){toast(e.message,'error')}})}
...
async function renderGuestDashboard(){
  const [d,areas,s]=await Promise.all([api("/api/dashboard"),api("/api/areas"),api("/api/slots")]);
  window.__latestSlots=s;
  const grouped=[...new Map(s.map(x=>[x.area_id,{id:x.area_id,name:x.area_name,slots:[]}])).values()];
  s.forEach(x=>grouped.find(a=>a.id===x.area_id)?.slots.push(x));
  const empty=d.empty, occupied=d.occupied, total=d.total_slots;
  const areaCards=areas.map(a=>`<div class="card guest-area-card"><div class="label">${a.name.toUpperCase()}</div><div class="metric">${a.empty}</div><div class="trend">chỗ trống / ${a.capacity} vị trí</div><div class="guest-mini-bar"><span style="width:${a.capacity?Math.min(100,a.occupied/a.capacity*100):0}%"></span></div></div>`).join("");
  const zone=ar=>{const occ=ar.slots.filter(x=>x.status==="occupied").length,emp=ar.slots.length-occ;const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a";return `<div class="map-zone ${cls}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ bãi</h3><span class="muted">${ar.slots.length} vị trí · ${emp} trống · ${occ} đang dùng</span></div><span class="pill ${emp?"green":"red"}">${emp?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid guest-slot-grid">${ar.slots.map(x=>{const vt=x.vehicle_type||"";const vcls=vt.toLowerCase().includes("đạp")?"vehicle-bicycle":(vt.toLowerCase().includes("máy")?"vehicle-bike":"vehicle-car");return `<div class="real-slot guest-slot ${x.status} ${x.status==="occupied"?vcls:""}"><div class="slot-top"><span>${x.name}</span><span>${x.status==="occupied"?vt:""}</span></div><div class="slot-icon">${x.status==="occupied"?vehicleIcon(vt):emptyIcon()}</div>${x.status==="occupied"?`<span class="plate">${displayPlate(x.license_plate,x.vehicle_type)}</span>`:`<span class="slot-status">CHỖ TRỐNG</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG SỬ DỤNG":"SẴN SÀNG"}</span></div>`}).join("")}</div></div>`};
  $("#pageTitle").textContent="Tình trạng bãi";
  $("#content").innerHTML=`<div class="notice guest-notice"><b>CHẾ ĐỘ KHÁCH · CHỈ XEM</b> · Tình trạng được cập nhật từ dữ liệu thực tế. Bạn có thể xem sơ đồ từng vị trí nhưng không thể thay đổi dữ liệu.</div><div class="cards guest-cards"><div class="card"><div class="label">CÒN TRỐNG</div><div class="metric">${empty}</div><div class="trend">trên tổng ${total} vị trí</div></div><div class="card"><div class="label">ĐANG SỬ DỤNG</div><div class="metric">${occupied}</div><div class="trend">vị trí đã có xe</div></div><div class="card"><div class="label">TỶ LỆ LẤP ĐẦY</div><div class="metric">${total?Math.round(occupied/total*100):0}%</div><div class="trend">đang sử dụng</div></div></div><div class="panel"><div class="panel-head"><div><h3>Chỗ trống theo khu</h3><span class="muted">Theo dõi nhanh tình trạng từng khu</span></div><span class="pill green">LIVE</span></div><div class="cards guest-area-grid">${areaCards}</div></div><div class="panel guest-map-panel"><div class="panel-head"><div><h3>🗺️ Sơ đồ bãi xe</h3><span class="muted">Xanh = chỗ trống · Đỏ = đang sử dụng · 🚲 = xe đạp</span></div><button class="btn" id="guestRefreshMap">↻ Cập nhật</button></div><div class="area-list guest-area-list">${grouped.map(zone).join("")}</div></div>`;
  $("#guestRefreshMap")?.addEventListener("click",renderGuestDashboard);
}
