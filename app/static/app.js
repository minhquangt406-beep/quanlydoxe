...
async function boot(){if(!token)return;try{me=await api("/api/me");$("#loginView").classList.add("hidden");$("#appView").classList.remove("hidden");const isManager=me.role==="manager",isGuest=me.role==="guest";$("#userName").textContent=me.full_name;$("#userRole").textContent=isManager?"Quản lý":(isGuest?"Khách xem bãi":"Nhân viên");$("#avatar").textContent=(me.full_name||"K")[0];$("#sidebarUserName").textContent=me.full_name;$("#sidebarUserRole").textContent=isManager?"Quản trị viên":(isGuest?"Khách · Chỉ xem chỗ trống":"Nhân viên bãi xe");$("#sidebarUser")?.classList.toggle("manager-profile",isManager);$("#sidebarUser")?.classList.toggle("staff-profile",!isManager&&!isGuest);$("#sidebarUser")?.classList.toggle("guest-profile",isGuest);$$('.manager-only').forEach(x=>x.style.display=isManager?"flex":"none");$$('.sidebar nav button').forEach(x=>{if(isGuest)x.style.display=x.dataset.page==="dashboard"?"flex":"none"});$("#mobileBottomNav")?.classList.toggle("manager",isManager);$("#mobileBottomNav")?.querySelectorAll("button").forEach(x=>{if(isGuest)x.style.display=x.dataset.page==="dashboard"?"flex":"none"});if(isGuest){await renderGuestDashboard()}else{await navigate("dashboard")}}catch(e){clearAuth()}}

const showRegister=document.getElementById("showRegister"), registerForm=document.getElementById("registerForm"), loginFormEl=document.getElementById("loginForm"), cancelRegister=document.getElementById("cancelRegister");
showRegister?.addEventListener("click",()=>{loginFormEl?.classList.add("hidden");showRegister.parentElement?.classList.add("hidden");registerForm?.classList.remove("hidden");document.getElementById("registerError").textContent="";});
cancelRegister?.addEventListener("click",()=>{registerForm?.classList.add("hidden");loginFormEl?.classList.remove("hidden");showRegister.parentElement?.classList.remove("hidden");});
registerForm?.addEventListener("submit",async e=>{e.preventDefault();const err=document.getElementById("registerError");err.textContent="";try{await api("/api/auth/register",{method:"POST",body:{full_name:document.getElementById("registerName").value,username:document.getElementById("registerUsername").value,password:document.getElementById("registerPassword").value}});err.textContent="Đăng ký thành công. Hãy đăng nhập bằng tài khoản vừa tạo.";err.className="success";registerForm.reset();}catch(e){err.textContent=e.message;err.className="error";}});
...
async function renderGuestDashboard(){const d=await api("/api/dashboard"),areas=await api("/api/areas");$("#pageTitle").textContent="Tình trạng bãi";const areaCards=areas.map(a=>`<div class="card guest-area-card"><div class="label">${a.name.toUpperCase()}</div><div class="metric">${a.empty}</div><div class="trend">chỗ trống / ${a.capacity} vị trí</div></div>`).join("");$("#content").innerHTML=`<div class="panel guest-welcome"><div class="panel-head"><div><h3>🚗 Tình trạng chỗ đỗ</h3><span class="muted">Tài khoản khách chỉ có quyền xem tình trạng chỗ trống.</span></div><span class="pill green">CHỈ XEM</span></div><div class="cards guest-cards"><div class="card"><div class="label">CÒN TRỐNG</div><div class="metric">${d.empty}</div><div class="trend">trên tổng ${d.total_slots} vị trí</div></div><div class="card"><div class="label">ĐANG SỬ DỤNG</div><div class="metric">${d.occupied}</div><div class="trend">vị trí đã có xe</div></div><div class="card"><div class="label">TỔNG VỊ TRÍ</div><div class="metric">${d.total_slots}</div><div class="trend">toàn bãi</div></div></div></div><div class="panel"><div class="panel-head"><div><h3>Chỗ trống theo khu</h3><span class="muted">Cập nhật theo dữ liệu thực tế</span></div></div><div class="cards guest-area-grid">${areaCards}</div></div>`;}
