...
const zone=(ar)=>{const occupied=ar.slots.filter(x=>x.status==="occupied").length,empty=ar.slots.length-occupied; const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a"; return `<div class="map-zone ${cls}" data-area-id="${ar.id}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ bãi</h3><span class="muted">${ar.slots.length} vị trí · ${empty} trống · ${occupied} đang dùng</span></div><span class="pill ${empty?"green":"red"}">${empty?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid">${ar.slots.map(x=>`<div class="real-slot ${x.status}" data-slot-id="${x.id}" data-area-id="${ar.id}"><div class="slot-top"><span>${x.name}</span><span>${x.vehicle_type||"—"}</span></div><div class="slot-icon">${x.status==="occupied"?(String(x.vehicle_type||"").toLowerCase().includes("ô tô")||String(x.vehicle_type||"").toLowerCase().includes("oto")?"🚗":"🏍️"):"▱"}</div>${x.status==="occupied"?`<span class="plate">${formatPlate(x.license_plate)||"NO PLATE"}</span>`:`<span class="slot-status empty-label">Chỗ trống</span><span class="slot-hint">Bấm để thêm xe</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG ĐỖ":""}</span></div>`).join("")}</div></div>`};
...
slots:async()=>{let s=await api("/api/slots");window.__latestSlots=s; const areas=[...new Map(s.map(x=>[x.area_id,{id:x.area_id,name:x.area_name,slots:[]}])).values()]; s.forEach(x=>areas.find(ar=>ar.id===x.area_id)?.slots.push(x)); const zone=(ar)=>{const occupied=ar.slots.filter(x=>x.status==="occupied").length,empty=ar.slots.length-occupied;const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a";return `<div class="panel area-panel-large"><div class="parking-map"><div class="map-zone ${cls}" data-area-id="${ar.id}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ vị trí</h3><span class="muted">${ar.slots.length} vị trí · ${empty} trống · ${occupied} đang dùng</span></div><span class="pill ${empty?"green":"red"}">${empty?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid">${ar.slots.map(x=>`<div class="real-slot ${x.status}" data-slot-id="${x.id}" data-area-id="${ar.id}"><div class="slot-top"><span>${x.name}</span><span>${x.vehicle_type||"—"}</span></div><div class="slot-icon">${x.status==="occupied"?(String(x.vehicle_type||"").toLowerCase().includes("ô tô")||String(x.vehicle_type||"").toLowerCase().includes("oto")?"🚗":"🏍️"):"▱"}</div>${x.status==="occupied"?`<span class="plate">${formatPlate(x.license_plate)||"NO PLATE"}</span>`:`<span class="slot-status empty-label">Chỗ trống</span><span class="slot-hint">Bấm để thêm xe</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG ĐỖ":""}</span></div>`).join("")}</div></div></div></div>`}; $("#content").innerHTML=`<div class="notice">Bản đồ được chia riêng theo từng khu. <b>Biển số</b> nằm ngay trên ô đang sử dụng.</div><div class="area-list">${areas.map(zone).join("")}</div>`},
