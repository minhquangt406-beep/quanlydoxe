...
const rowHtml=row=>`<tr><td>#${row.id}</td><td><b>${formatPlate(row.license_plate)}</b></td><td>${row.vehicle_type}</td><td>${row.slot}</td><td>${dt(row.time_in)}</td><td>${dt(row.time_out)}</td><td>${row.fee!=null?money(row.fee):"—"}</td><td><span class="payment-badge ${methodClass(methodKey(row))}">${methodIcon(methodKey(row))} ${methodLabel(methodKey(row))}</span></td><td>${row.paid_at?dt(row.paid_at):"—"}</td><td><span class="pill ${row.time_out?"green":"red"}">${row.time_out?"Đã ra":"Đang gửi"}</span></td>${me?.role==="manager"?`<td><button class="btn danger delete-history" data-id="${row.id}">🗑 Xóa</button></td>`:""}</tr>`;
