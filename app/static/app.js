...
async function boot(){if(!token)return;try{me=await api("/api/me");$("#loginView").classList.add("hidden");$("#appView").classList.remove("hidden");const isManager=me.role==="manager";$("#userName").textContent=me.full_name;$("#userRole").textContent=isManager?"Quản lý":"Nhân viên";$("#avatar").textContent=me.full_name[0];$("#sidebarUserName").textContent=me.full_name;$("#sidebarUserRole").textContent=isManager?"Quản trị viên":"Nhân viên bãi xe";$("#sidebarUser")?.classList.toggle("manager-profile",isManager);$("#sidebarUser")?.classList.toggle("staff-profile",!isManager);$$(".manager-only").forEach(x=>x.style.display=isManager?"flex":"none");$("#mobileBottomNav")?.classList.toggle("manager",isManager);await navigate("dashboard")}catch(e){clearAuth()}}
