...
async function openPaymentModal(recordId, closeAfter){
  const m=$("#slotModal"),b=$("#modalBody"); if(!m||!b)return;
  try{
    const d=await api(`/api/checkout-preview/${recordId}`);
    b.innerHTML=`<div class="eyebrow">THANH TOÁN & XE RA</div>
      ...
      <div class="payment-methods">
        <button class="payment-method active" data-method="Tiền mặt"><span>💵</span><b>Tiền mặt</b><small>Thu trực tiếp</small></button>
        <button class="payment-method" data-method="Chuyển khoản"><span>🏦</span><b>Chuyển khoản</b><small>Ngân hàng</small></button>
        <button class="payment-method" data-method="QR ngân hàng"><span>📱</span><b>Quét QR</b><small>VietQR / NAPAS</small></button>
        <button class="payment-method" data-method="Miễn phí"><span>🆓</span><b>Miễn phí</b><small>Không thu tiền</small></button>
      </div>
      <div id="paymentQR" class="payment-qr hidden"><img src="/static/images/payment-qr.jpg" alt="QR thanh toán"><div><b>Quét mã để chuyển khoản</b><small>Techcombank · NONG MINH QUANG<br>9988 8805 6789</small></div></div>
      <div class="payment-note">Sau khi xác nhận, hệ thống lưu phương thức thanh toán vào lịch sử.</div>
      <div class="modal-actions"><button class="btn" id="paymentCancel">Hủy</button><button class="primary" id="paymentConfirm">Xác nhận ${money(d.fee)}</button></div>`;
    m.classList.remove("hidden"); document.body.classList.add("modal-open");
    const close=()=>{m.classList.add("hidden");document.body.classList.remove("modal-open");};
    $("#modalClose").onclick=close; $(".modal-backdrop").onclick=close; $("#paymentCancel").onclick=close;
    let method="Tiền mặt";
    $$(".payment-method").forEach(btn=>btn.onclick=()=>{method=btn.dataset.method;$$('.payment-method').forEach(x=>x.classList.toggle('active',x===btn));$("#paymentQR").classList.toggle("hidden",method!=="QR ngân hàng");$("#paymentConfirm").textContent=`Xác nhận ${money(d.fee)}`;});
    $("#paymentConfirm").onclick=async()=>{const btn=$("#paymentConfirm");btn.disabled=true;btn.textContent="Đang xử lý...";try{const out=await api("/api/checkout",{method:"POST",body:{record_id:recordId,payment_method:method}});close();toast(`✓ Đã thanh toán ${money(out.fee)} · ${method}`);await navigate("dashboard");}catch(e){toast(e.message,"error");btn.disabled=false;btn.textContent=`Xác nhận ${money(d.fee)}`;}};
  }catch(e){toast(e.message,"error");}
}
...
const zone=(ar)=>{const occupied=ar.slots.filter(x=>x.status==="occupied").length,empty=ar.slots.length-occupied; const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a"; return `<div class="map-zone ${cls}" data-area-id="${ar.id}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ bãi</h3><span class="muted">${ar.slots.length} vị trí · ${empty} trống · ${occupied} đang dùng</span></div><span class="pill ${empty?"green":"red"}">${empty?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid">${ar.slots.map(x=>`<div class="real-slot ${x.status}" data-slot-id="${x.id}" data-area-id="${ar.id}"><div class="slot-top"><span>${x.name}</span><span>${x.vehicle_type||"—"}</span></div><div class="slot-icon">${x.status==="occupied"?"🚗":"▱"}</div>${x.status==="occupied"?`<span class="plate">${formatPlate(x.license_plate)||"NO PLATE"}</span>`:`<span class="slot-status">CHỖ TRỐNG</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG SỬ DỤNG":"SẴN SÀNG"}</span></div>`).join("")}</div></div>`};
...
$$('.checkout').forEach(b=>b.onclick=()=>openPaymentModal(+b.dataset.id));nst $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const AWAY_TIMEOUT_MS=30*60*1000;
let token=localStorage.getItem("parking_token"), me=null;
function clearAuth(){localStorage.removeItem("parking_token");localStorage.removeItem("parking_last_seen");token=null;}
function refreshLastSeen(){if(token) localStorage.setItem("parking_last_seen",String(Date.now()));}
...
bindVehicleTypeDetection("#plate","#vtype");$("#checkin").onclick=async()=>{try{let d=await api("/api/checkin",{method:"POST",body:{license_plate:$("#plate").value,vehicle_type:$("#vtype").value,slot_id:+$("#slot").value}});$("#parkingMsg").innerHTML=`<div class="notice">✓ ${d.message} · Mã lượt <b>#${d.record_id}</b> · ${d.slot}</div>`;await pages.parking()}catch(e){$("#parkingMsg").innerHTML=`<div class="error">${e.message}</div>`}};
$("#areaFilter").onchange=()=>{const area=$("#areaFilter").value; $$("#slot option").forEach(o=>o.hidden=area!=="all" && o.dataset.area!==area); const first=$$("#slot option").find(o=>!o.hidden); if(first) $("#slot").value=first.value;};
$$(".checkout").forEach(b=>b.onclick=async()=>{if(!confirm("Xác nhận cho xe ra?"))return;try{let d=await api("/api/checkout",{method:"POST",body:{record_id:+b.dataset.id,payment_method:prompt("Phương thức: Tiền mặt / Chuyển khoản / QR ngân hàng / Miễn phí","Tiền mặt")||"Tiền mặt"}});alert(`Phí: ${money(d.fee)}\\nThời gian: ${d.hours} giờ`);await pages.parking()}catch(e){alert(e.message)}})},
slots:async()=>{let s=await api("/api/slots");window.__latestSlots=s; const areas=[...new Map(s.map(x=>[x.area_id,{id:x.area_id,name:x.area_name,slots:[]}])).values()]; s.forEach(x=>areas.find(ar=>ar.id===x.area_id)?.slots.push(x)); const zone=(ar)=>{const occupied=ar.slots.filter(x=>x.status==="occupied").length,empty=ar.slots.length-occupied;const cls=ar.name.toLowerCase().includes("b")?"zone-b":"zone-a";return `<div class="panel area-panel-large"><div class="parking-map"><div class="map-zone ${cls}" data-area-id="${ar.id}"><div class="zone-title"><div><span class="zone-chip">${ar.name.toUpperCase()}</span><h3>${ar.name} · Sơ đồ vị trí</h3><span class="muted">${ar.slots.length} vị trí · ${empty} trống · ${occupied} đang dùng</span></div><span class="pill ${empty?"green":"red"}">${empty?"CÒN CHỖ":"ĐẦY"}</span></div><div class="real-slot-grid">${ar.slots.map(x=>`<div class="real-slot ${x.status}" data-slot-id="${x.id}" data-area-id="${ar.id}"><div class="slot-top"><span>${x.name}</span><span>${x.vehicle_type||"—"}</span></div><div class="slot-icon">${x.status==="occupied"?"🚗":"▱"}</div>${x.status==="occupied"?`<span class="plate">${formatPlate(x.license_plate)||"NO PLATE"}</span>`:`<span class="slot-status">CHỖ TRỐNG</span>`}<span class="slot-status">${x.status==="occupied"?"ĐANG SỬ DỤNG":"SẴN SÀNG"}</span></div>`).join("")}</div></div></div></div>`}; $("#content").innerHTML=`<div class="notice">Bản đồ được chia riêng theo từng khu. <b>Biển số</b> nằm ngay trên ô đang sử dụng.</div><div class="area-list">${areas.map(zone).join("")}</div>`},
