...
function renderAreaAvailability(areas){
  ...
  const rows=(areas||[]).filter(ar=>ar&&ar.name).map(ar=>{
    ...
    const total=Number(ar.capacity ?? slots.length ?? 0);
    const occupied=Number(ar.occupied ?? slots.filter(x=>x.status==='occupied').length);
    const empty=Math.max(0,Number(ar.empty ?? (total-occupied)));
    return {id:ar.id,name:ar.name,total,occupied,empty,rate:total?Math.round(occupied/total*100):0};
  });
  wrap.innerHTML=rows.length?rows.map(x=>`
    <div class="simple-area-card" data-area-id="${x.id}">
      <div class="simple-area-card-head">
        <b>${escapeHtml(x.name)}</b>
        <span>${x.empty} chỗ trống</span>
      </div>
